...
from app.models import Permission, Folder, User, OktaUser
...
class Okta_PermissionService:
    ...
    def check_folder_permission(
        ...
        # Owner has all permissions
        if folder.owner_id == okta_user_id:
            return True
        
        # Check direct permissions
        permission = self.db.query(Permission).filter(
            Permission.user_id == okta_user_id,
            Permission.folder_id == folder_id
        ).first()
        ...
        # Check parent folder permissions (inheritance)
        if folder.parent_id:
            return self.check_folder_permission(okta_user_id, folder.parent_id, permission_type)
        
        return False

    def get_user_accessible_folders(self, okta_user_id: str) -> List[Folder]:
        """Get all folders accessible to user"""
        # Check if user is superuser first
        user = self.db.query(OktaUser).filter(OktaUser.okta_user_id == okta_user_id).first()
        if not user:
        ...
        # Get folders owned by user
        owned_folders = self.db.query(Folder).filter(Folder.owner_id == okta_user_id).all()
    ...
    def grant_permission(
        ...
        """Grant permission to user for folder"""
        # Check if granter is superuser
        granter = self.db.query(OktaUser).filter(OktaUser.okta_user_id == granter_id).first()
        if not (granter and granter.is_superuser):
            # If not superuser, check if granter has admin rights
            if not self.check_folder_permission(granter_id, folder_id, "admin"):
                folder = self.db.query(Folder).filter(Folder.id == folder_id).first()
    ...
    def revoke_permission(
        ...
        """Revoke user's permission for folder"""
        # Check if revoker is superuser
        revoker = self.db.query(OktaUser).filter(OktaUser.okta_user_id == revoker_id).first()
        if not (revoker and revoker.is_superuser):
            # If not superuser, check if revoker has admin rights
            folder = self.db.query(Folder).filter(Folder.id == folder_id).first()
            if not folder:
    ...
    def check_folder_access(self, okta_user_id: str, folder_id: UUID, permission_type: str = "read"):
        """Check folder access and raise exception if denied"""
        if not self.check_folder_permission(okta_user_id, folder_id, permission_type):
            raise PermissionDeniedException(f"You don't have {permission_type} permission for this folder")
